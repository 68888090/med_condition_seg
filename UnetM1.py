...
class UnetM(nn.Module):
    def __init__(self, 
        ...
        # text_channels = [768, 768, 768, 768, 768]
        # === 【核心修改 1】替换为混合融合模块 ===
        # 移除 MultiScaleQueryFusion 和 CrossScaleFusion
        self.fusion_model = HybridFusionModule(
    ...
    def forward(self, img1, img2, textout):
        ...
        with torch.no_grad():
            ...
            text = self.text_processor(textout) # [B, 60, 768]
        ...
        # 且保留了原始空间分辨率 (D, H, W)
        fused_skips = self.fusion_model(img1_out, img2_out, text)
